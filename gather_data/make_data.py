...
def make_initial_dataset(domain, num, formatting_instructions=None):
    ...
    messages = [{"role": "system", "content": f"You are a {domain} expert"}]
    messages.append(
...
num_sim = 10
prompt = (
    f"You are a {domain} expert system. You will be given names of {domain}s, and you will respond with a list of "
    ...
    + f" [list of similar {domain}s ...]}}. ONLY return {domain}s. returned {domain}s"
    ...
    for e in entities:
        try:
            ...
            for i in range(len(d.keys())):
                ...
                new_entities += [
                    D
                    for D in list(d.values())[i]
                    if D not in entities
                    and D not in searched_entities
                    and D not in new_entities
                ]
