...
parser.add_argument(
    "--domain_to", help="whats the domain to map to!?!?!?!!", required=True
)
parser.add_argument(
    "--domain_from", help="whats the domain to map from!?!?!?!!", required=True
)
...
if domain_to == "movie":
    formatting_instructions = (
        "Movies should be in the form name_of_movie (release date)."
    )
elif domain_to == "song":
    formatting_instructions = (
        "Songs should be in the form name_of_song|artist_name(release date)."
    )
elif domain_to == "book":
    formatting_instructions = (
        "Books should be in the form name_of_book|author_name(release date)."
    )
else:
...
num_sim = 5
prompt = (
    f"You are a both a {domain_to} and a {domain_from} expert system. You will be given names of {domain_from}s, and you will respond with a list of "
...
corpus = f.read()
sentence_list = [
    lst.replace(" ", "_").replace("&", "and") for lst in corpus.lower().split("\n")
]
