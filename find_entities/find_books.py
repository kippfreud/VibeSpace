...
for e in entity_list:
    ...
    for item in data_first["items"]:
        ...
        shared_words = [
            ...
            for w in [
                ...
                for w in raw_tit.split(" ")
            ]
            if w
            in [
                i.replace(":", "").replace("(", "").replace(")", "")
                for i in item_tit.lower().split(" ")
            ]
        ]
        if remove_stopwords(raw_tit) == "" or remove_stopwords(raw_tit) == "":
            ...
            shared_words = [
                s
                for s in remove_stopwords(" ".join(shared_words)).split(" ")
                if s != ""
            ]
    ...
    real += 1
    print(
        f"So far {real} Real ({cats} with genres, {no_cats} without) and {fake} Fake: {round(100 * real / (fake + real), 2)}% Accuracy so far..."
    )
    print(f"{e} -> {name}")
