...
class MapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(input, device=self._device), torch.tensor(
            out, dtype=torch.float32, device=self._device
        )
...
def train_AE_mapper(
    ...
    model = Autoencoder(vibe_vector_size).to(device)
    ...
    train_dataset = MapDataset(
        dataset[: round(len(dataset) * train_raio)], device=device
    )
    val_dataset = MapDataset(dataset[round(len(dataset) * train_raio) :], device=device)
    ...
    if loss_func == "cosine":
        ...
        def cos_loss(a, b):
            return cos_emb_loss(a, b, torch.ones(a.size(0)).to(device))
    ...
    for epoch in range(num_epochs):
        ...
        for data, labels in train_loader:
            ...
            for i in range(50):
                ...
                d = d + torch.empty(d.size(), device=device).normal_(mean=0, std=sd)
                l = l + torch.empty(l.size(), device=device).normal_(mean=0, std=sd)
